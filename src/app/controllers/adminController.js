...
class adminController {
    ...
    async addTracksToCourse(req, res) {
        try {
            ...
            const trackStepsPromises = trackData.track_steps.map(
                async (stepData) => {
                    let videoDuration = stepData.video.duration;

                    if (stepData.video.url && !videoDuration) {
                        try {
                            videoDuration = await getYouTubeVideoDuration(
                                stepData.video.url,
                            );
                        } catch (err) {
                            console.error(
    ...
    async removeTrack(req, res) {
        ...
        try {
            ...
            await TrackStep.deleteMany({
                _id: { $in: deletedTrack.track_steps },
            });
    ...
    async updateUser(req, res) {
        try {
            ...
            if (typeof is_admin !== 'boolean') {
                return res
                    .status(400)
                    .json({ message: 'is_admin must be a boolean' });
            }
            ...
            if (userUpdateResult.modifiedCount === 1) {
                ...
                if (courseUpdateResult.modifiedCount === 1) {
                    ...
                    if (progressRemovalResult.deletedCount >= 0) {
                        ...
                        res.status(500).send({
                            message:
                                'Progress removal failed. Updates rolled back.',
                        });
                    }
                } else {
